...
function getData() {
    ...
    let arrayData = JSON.parse(localStorage.getItem("drinks"));
    ...
    if (arrayData !== null) {
        for (let i = 1; i < arrayData.length; i++) {
          ...
          new Drink(arrayData[i].name, arrayData[i].ingredients,
            arrayData[i].image, arrayData[i].isCold, 
            arrayData[i].isHot, arrayData[i].price);
            }
